...
(function () {
  ...
  angular
    ...
    .controller('PartnerQuicksearchController', PartnerQuicksearchController);

  /** @ngInject */
  function PartnerQuicksearchController($log, $rootScope, $state, $window, $filter, jQ, localStorage, player, partner, pagination, common) {
    var vm = this;
    ...
    if(lstore){
      ...
      // game.emit('getProviderList', {playerId:lstore.player.playerId}, 99);
    }
    ...
    vm.searchTransaction = function(initSearch){
        ...
        partner.emit('getPartnerPlayerRegistrationReport',{
    ...
    partner.on('getPartnerPlayerRegistrationReport',function(data){
      if(data.data){
          if(data.data.players){
            vm.registerUserAnalysis = data.data.players;
          }
          if(data.data.stats.totalCount){
            vm.registerUserAnalysisStats = data.data.stats.totalCount;
          }
          var dataLength = data.data.stats.totalCount;
          vm.totalLength = data.data.stats.totalCount;
          vm.totalPg = pagination.buildPagin(dataLength, vm.pgLimit);
          if(vm.tab2pg==1){
              vm.pickPage(1);
          }
      }
